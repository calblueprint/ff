...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (donation.mealPhoto)
    {
        ...
        UIImage *mealPhotoThumbImage = [[FFImageStore sharedStore] imageForKey:donation.mealPhoto.thumbnailURL];
        
        // If the image store doesn't have it, download it asynchronously.
        if (!mealPhotoThumbImage)
        {
            NSURLRequest *request = [NSURLRequest requestWithURL:[NSURL URLWithString:donation.mealPhoto.thumbnailURL]];
            ...
            [cell.imageViewMealPhoto setImageWithURLRequest:request placeholderImage:nil success:^(NSURLRequest *request, NSHTTPURLResponse *response, UIImage *image) {
                ...
                [[FFImageStore sharedStore] setImage:image forKey:donation.mealPhoto.thumbnailURL];
                // Show meal photo
                ...
                // Use default image
                [strongCell.imageViewMealPhoto setImage:[UIImage imageNamed:kCurrentDonationsDefaultMealPhoto]];
            }];
        }
        else {
            [cell.imageViewMealPhoto setImage:mealPhotoThumbImage];
        }
    }
    else {
        // Use default image
        [cell.imageViewMealPhoto setImage:[UIImage imageNamed:kCurrentDonationsDefaultMealPhoto]];
    }
